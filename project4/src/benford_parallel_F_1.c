 ...
 * task 2 F_1 (creating a 1d array)
 *
...
There are 495 8's
There are 477 9's
It took 0.000217 seconds for the whole thing to run
...
There are 51298 8's
There are 46745 9's
It took 0.007700 seconds for the whole thing to run
...
void* thrCount(void* arg){
    ...
    for(int i = 0; i<thr_data->len;i++){
        ...
        threads_global_counts[(leading_digit*(NUM_THREADS-1))+leading_digit+thr_data->tid]++;
...
//      succ = loadData( "bin/medium.bin" );
    ...
    for (int j = 0; j<10;j++) {
        for(int i = 0; i < numThreads; i++){
            global_counts[j] += threads_global_counts[(j*(NUM_THREADS-1))+j+i];
        };
    }
